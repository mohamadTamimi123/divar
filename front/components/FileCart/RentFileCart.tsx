                ...
                <div className="flex flex-col flex-1 justify-between p-6">
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-sm">
                            {/* Metraj */}
                            {(props.metraj || props.rentDetail?.metraj) && (
                                <div className="flex items-center gap-1 text-gray-600">
                                    ...
                                    <span className="font-medium">{props.metraj || props.rentDetail?.metraj} متر</span>
                                </div>
                            )}
                            
                            {/* Floor */}
                            {(props.tabagheCurrent || props.rentDetail?.tabagheCurrent) && (
                                <div className="flex items-center gap-1 text-gray-600">
                                    <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="text-gray-400">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                    </svg>
                                    <span className="font-medium">طبقه {props.tabagheCurrent || props.rentDetail?.tabagheCurrent}</span>
                                </div>
                            )}
                            
                            {/* Build Year */}
                            {(props.buildYear || props.rentDetail?.buildYear) && (
                                <div className="flex items-center gap-1 text-gray-600">
                                    <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="text-gray-400">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                                    <span className="font-medium">{props.buildYear || props.rentDetail?.buildYear}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="mt-6 space-y-3">
                        ...
                        {(props.deposit || props.rent || props.vadie || props.ejare) && (
                            <div className="space-y-2">
                                {(props.deposit || props.vadie) && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-500">ودیعه:</span>
                                        <span className="font-semibold text-purple-600">{formatPrice(props.deposit || props.vadie)}</span>
                                    </div>
                                )}
                                {(props.rent || props.ejare) && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-gray-500">اجاره:</span>
                                        <span className="font-semibold text-gray-900">{formatPrice(props.rent || props.ejare)}</span>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {/* Legacy Price */}
                        {props.price && !props.deposit && !props.rent && !props.vadie && !props.ejare && (
                            <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                                <span className="text-xs text-gray-500 font-medium">قیمت کل</span>
                                <span className="text-lg font-bold text-purple-600">{formatPrice(props.price)}</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button 
                                ...
                                className="flex-1 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg transition-colors duration-200"
                            >
                                ورود
                            </button>
                        </div>
